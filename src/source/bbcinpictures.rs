...
impl BBCInPictures {
    pub fn new(path: &Path) -> BBCInPictures {
        let mut source = BBCInPictures { remote_entries: HashMap::new(), entries: HashMap::new() };
...
impl Source for BBCInPictures {
    ...
    fn name(&self) -> String { String::from("BBCInPictures") }

    fn base_url(&self) -> String { String::from("https://www.bbc.co.uk/news/in_pictures") }
        ...
        for listitem in document.select(&li_selector) {
            ...
            for div in listitem.select(&div_selector) {
                ...
                match div.text().collect::<Vec<_>>().first() {
                    Some(t) => { if *t != "Posted" { continue }},
                    None => continue
                };

                let parent = match div.parent() {
                    Some(p) => {
                        match ElementRef::wrap(p) {
                            Some(p) => p,
                            None => continue
                        }
                    },
                ...
                for span in parent.select(&span_selector) {
                    if span.attr("aria-hidden").is_some() {
                        ...
                        let date: NaiveDate = if sdate.contains(" ") {
                            match NaiveDate::parse_from_str(&format!("{} {}", sdate, now.year()), "%d %b %Y") {
                                Ok(d) => d,
                                Err(_) => continue
                            }
                        }
                            ...
                            match now.checked_sub_signed(delta) {
                                Some(time) => {
                                    match NaiveDate::from_ymd_opt(time.year(), time.month(), time.day()) {
                                        Some(date) => date,
                                        None => continue
                                    }
                                },
                        ...
                        for img in picture.select(&img_selector) {
                            match img.attr("src") {
                                Some(src) => { picture_url = src.to_string(); break },
                                None => {}
                            }
                        }
                        add_entry(&mut self.remote_entries, date, title, url.to_string(), Some(picture_url));
                    }
                }
                if found { break; }
            }
